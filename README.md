...
各処理段階で生成されるデータのサイズ見積もり（実測値ベース）。

### 第1段階: データ収集（実測値）

| 期間 | データサイズ | 内訳 |
|------|------------|------|
| **1ヶ月** | 約82MB | バーデータ: 約2MB（5TF × 8列 × int64/float32 × 約37,000行/TF） + Tickデータ: 約80MB（約200-400万件） |
| **6ヶ月** | 約491MB | バーデータ: 約12MB + Tickデータ: 約479MB（13,840,269件実測） |
| **7年** | 約6.9GB | バーデータ: 約168MB + Tickデータ: 約6.7GB（推定） |

### 各処理段階のストレージ要件（1ヶ月分の見積もり）

| 処理段階 | 出力ファイル | データサイズ（1ヶ月） | 説明 |
|---------|-------------|---------------------|------|
| 第1段階: データ収集 | `models/*_data_collector.h5` | 約82MB | バーデータ（5TF）: 約2MB + Tickデータ: 約80MB（実測） |
| 第2段階: 特徴量計算 | `models/*_features.h5` | 約1.2GB | 5TF × 50-80列 × float32 × 約45,000行（推定） |
| 第3段階: 前処理 | `models/*_preprocessed.h5` | 約2.5GB | シーケンス化: M1(480)×M5(288)×M15(192)×H1(96)×H4(48) × 50-80特徴（推定） |
...
**合計ストレージ要件（1ヶ月分）**: 約3.8GB（実測+推定）
**推奨ディスク容量**: 20GB以上（複数期間の保存、ログ、バックアップを考慮）

**データ保持ポリシー**:
- 生データ（第1段階）: 3ヶ月保持（約246MB）
- 前処理済みデータ（第3段階）: 最新1ヶ月のみ保持（約2.5GB）
- 学習済みモデル: 全バージョン保持（バージョニング管理）

**注意**: 
- 第1段階は実測値、第2-6段階は推定値（未実装）
- Tickデータが全体の大部分を占める（1ヶ月で約80MB）
- 月分割取得によりメモリ効率的に処理可能
