...
**重要**: MT5 API Serverは **既にUTC** でタイムスタンプを返却するため、タイムゾーン変換は不要。

**時刻管理方針**:
- **内部処理**: UTC統一で処理・保存（MT5 APIから直接取得）
- **ログ表示**: 日本時間(JST)で表示
...
**処理順序**: MT5 API Server → データ検証 → TIMESTAMP_ALIGNMENT

```
データ収集フロー（時刻処理）:

┌──────────────────────────────────────────────────────────────┐
│ 1. MT5 API Server からデータ取得                              │
│    - 既にUTCエポック秒/ミリ秒でタイムスタンプ取得             │
│    - mt5.copy_rates_range() → time: UTCエポック秒            │
│    - mt5.copy_ticks_range() → time/time_msc: UTC             │
│    - タイムゾーン変換不要                                      │
└──────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────┐
│ 2. データ検証: UTCタイムスタンプの整合性確認                  │
│    - 単調増加チェック                                         │
│    - 重複チェック                                             │
│    - 範囲チェック（指定期間内）                               │
└──────────────────────────────────────────────────────────────┘
