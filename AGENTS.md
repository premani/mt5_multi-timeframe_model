...
3. ❌ `except: pass` でエラー握りつぶしていないか
4. ✅ 提案→承認→実装の順で進めているか
5. ✅ コミットメッセージに誇張語・数値が入っていないか
6. ✅ Phase表記に内容説明があるか
7. ✅ テストファイル削除したか
8. ✅ `src/utils/` 編集時に仕様書更新したか
...
3. **エラー処理**: サイレントに握りつぶさず、必ず raise して伝播
4. **実装フロー**: 提案 → 確認 → 実装 → 検証の順守
5. **コミットメッセージ**: 日本語、AI署名禁止、誇張語禁止、具体的数値禁止
6. **Phase表記**: 必ず内容を明示（例: `NaN除外ロジック phase2`）
7. **テストファイル**: `_test_*.py` は検証後必ず削除
8. **共通モジュール**: `src/utils/` 編集時は `docs/utils/` の仕様書も同時更新
9. **ディレクトリ構造**: メインスクリプトと同名ディレクトリに固有モジュール配置